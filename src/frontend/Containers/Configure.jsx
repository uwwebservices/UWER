...
class Configure extends Component {
    ...
    if (this.validateGroupString(this.state.newSubgroup)) {
      ...
      if (success) {
        ...
        this.props._addNotification(
          "Registration Group Created",
          `Successfully created registration group: ${this.state.newSubgroup}`
        );
      } else {
        this.props._addNotification(
          "Create Registration Group Failed",
          "Group creation failed, does this group already exist?"
        );
      }
      this.setState({
        creatingGroup: false,
      ...
      this.props._addNotification(
        "Create Registration Group Failed",
        "Group name can only contain numbers, letters and spaces."
      );
    }
  };
  ...
  updateGroupName = async groupName => {
    ...
    this.props._addNotification(
      ...
      `Selected group successfully changed to: ${this.displayGroupName(
        groupName
      )}`,
      "success"
    );
  ...
  render() {
    ...
    return (
      <div>
        ...
        <ConfigOptions
          netidAllowed={this.state.netidAllowed}
          tokenTTL={this.state.tokenTTL}
        ...
        <div className="card">
          <div className="card-header">
            <h2 className="inline">
              Select a Registration Group{" "}
              <FA
                name="refresh"
                onClick={this.loadSubGroups}
                spin={this.state.loadingSubGroups}
              />
            </h2>
            <ContentModal
              openWithButton={true}
              ...
              approveButtonDisabled={
                this.state.invalidSubgroup || this.state.creatingGroup
              }
            ...
            >
              <div className="createSubgroupModal">
                <div>
                  ...
                  />
                  {this.state.invalidSubgroup &&
                    this.state.newSubgroup.length > 2 && (
                      <div className="subgroupError">
                        Registration groups must be longer than 2 characters and
                ...
                <div>
                  <input
                    type="checkbox"
                    name="newSubgroupEmailEnabled"
                    checked={this.state.newSubgroupEmailEnabled}
                    onChange={this.handleChange}
                  />
        ...
        <div className="card">
          ...
          <div className="card-body">
            <ul className="TOS">
              <li>
                *Application administrators are expected to have read and agreed
                to the{" "}
                <a
                  target="_blank"
                  href="https://uwnetid.washington.edu/agree/?accept=idaa&appid=idcardeventregistration"
                >
                  UW Institutional Data Access agreement
                </a>
                . Privacy expectations and appropriate use of personal data are
                the responsibility of the *Application administrators.
              </li>

              <li>
                The functionality contained in this application is explicitly
                for the use of capturing event participants. Any other use of
                the event participants’ information or Husky Card data elements
                (Name, NetID, Photo, Magstrip, RFID) information captured in the
                application is explicitly prohibited.
              </li>

              <li>
                The use of the “Allow NetID Registration” feature should always
                be accompanied by a secondary form of identification (e.g. UW or
                government issued ID). The use of the “Allow NetID Registration”
                feature without secondary identification <u>and</u> the event
                participant or registrant’s physical presence is explicitly
                prohibited.
              </li>

              <li>
                The “Private Group” feature should be used as the default
                setting and only *Application administrators should be given
                access to view the event participants.
              </li>

              <li>
                Use of the event participants’ information is to <u>only</u> be
                used for purposes directly related to the specific event the
                participant registered for or attended.
              </li>
            </ul>
            <br />
            <i>
              *<b>Application administrators</b> are those individuals that have
              been setup to access the configuration screen and are required to
              login. If you are reading this message you are an Application
              administrator.
            </i>
          </div>
        </div>
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    createGroup: (group, confidential, description, email) =>
      dispatch(CreateGroup(group, confidential, description, email)),
    startRegistrationSession: (groupName, netidAllowed, tokenTTL) =>
      dispatch(StartRegistrationSession(groupName, netidAllowed, tokenTTL)),
